...
module Orn
  module Tmux
    ...
    # Trust-gated worktree window opening. The trust check itself lives in
    # Orn::Trust; these verbs orchestrate it with session naming and
    # create_window.
    class Client
      # Open the tmux window for `branch` using the project's configured
      # layout, prompting for trust approval when the layout comes from an
      # untrusted source.
      ...
      # Open the window with an explicit layout and template variables,
      # checking trust for `layout_source` first.
      def open_window_with_layout(project, branch, layout, layout_source, template_vars: {})
      ...
      # untrusted project pane commands fail instead. For the TUI, where a
      # prompt would garble the screen.
      ...
      def open_checked_window(project, branch, layout, template_vars)
